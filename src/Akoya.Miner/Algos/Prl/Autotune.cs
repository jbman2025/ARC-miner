...
// (ARC_TGEMM_NB / _MB via getenv per dispatch; ARC_SEARCH_M at
...
internal static class Autotune
{
    ...
    private readonly record struct Config(int Nb, int Mb, int SearchM, int SearchN = 131072)
    {
        public override string ToString() => $"NB={Nb} MB={Mb} SEARCH_M={SearchM} SEARCH_N={SearchN}";
    }
    ...
    {
        ["A770"] = new Config(2, 2, 256, 131072),
        ["A750"] = new Config(2, 2, 256, 131072),
        ["A580"] = new Config(2, 2, 256, 65536),
        ["A380"] = new Config(2, 2, 256, 32768),
        ["B580"] = new Config(4, 2, 4096, 131072),
        ["B570"] = new Config(4, 2, 4096, 131072),
    };
    ...
    /// apply step.</summary>
    internal static (int Nb, int Mb, int SearchM, int SearchN)? ResolveTunedConfig(string cachePath, string sku)
    {
        if (TuneCache.Lookup(cachePath, sku) is { } t) return t;
        if (SkuDefaultFor(sku) is { } d) return (d.Nb, d.Mb, d.SearchM, d.SearchN);
        return null;
    }
    ...
    public static int Run(string[] args, ILoggerFactory lf, CancellationToken ct)
    {
        ...
        if (MinerEnv.Get("ARC_POOL_WALLET") is null)
            Environment.SetEnvironmentVariable("ARC_POOL_WALLET", "prl1autotune");
        MinerOptions opts;
        ...
        var savedNb = Environment.GetEnvironmentVariable("ARC_TGEMM_NB");
        var savedMb = Environment.GetEnvironmentVariable("ARC_TGEMM_MB");
        var savedSm = Environment.GetEnvironmentVariable("ARC_SEARCH_M");
        var savedSn = Environment.GetEnvironmentVariable("ARC_SEARCH_N");
        ...
        {
            Restore("ARC_TGEMM_NB", savedNb);
            Restore("ARC_TGEMM_MB", savedMb);
            Restore("ARC_SEARCH_M", savedSm);
            Restore("ARC_SEARCH_N", savedSn);
        }
        ...
        try
        {
            TuneCache.Write(cachePath, winners.Select(w =>
                (w.Gpu.Name, w.Best.Cfg.Nb, w.Best.Cfg.Mb, w.Best.Cfg.SearchM, w.Best.Cfg.SearchN, w.Best.Tmads)));
    ...
    ///
    /// Opt out with <c>--no-autotune</c> / <c>ARC_AUTOTUNE_ON_FIRST_RUN=0</c>;
    /// skipped if the user pinned any kernel knob (NB/MB/SEARCH_M/SEARCH_N). Never fatal —
        ...
        string[] args, MinerOptions opts, ILoggerFactory lf, CancellationToken ct)
    {
        ...
        if (MinerEnv.Get("ARC_AUTOTUNE_ON_FIRST_RUN") == "0")
            return;
        if (!string.IsNullOrEmpty(MinerEnv.Get("ARC_TGEMM_NB"))
            || !string.IsNullOrEmpty(MinerEnv.Get("ARC_TGEMM_MB"))
            || !string.IsNullOrEmpty(MinerEnv.Get("ARC_SEARCH_M"))
            || !string.IsNullOrEmpty(MinerEnv.Get("ARC_SEARCH_N")))
        {
        ...
        {
            log.LogInformation(
                "autotune: cached profile for \"{Sku}\" — NB={Nb} MB={Mb} SEARCH_M={Sm} SEARCH_N={Sn} "
                + "(run `arc-miner autotune` to re-tune)",
                sku, hit.Nb, hit.Mb, hit.SearchM, hit.SearchN);
            return;
        }
    ...
    {
        NativeEnv.Set("ARC_TGEMM_NB", c.Nb.ToString());
        NativeEnv.Set("ARC_TGEMM_MB", c.Mb.ToString());
        NativeEnv.Set("ARC_SEARCH_M", c.SearchM.ToString());
        NativeEnv.Set("ARC_SEARCH_N", c.SearchN.ToString());
    }
        ...
        string profileName, MineOptions shape)
    {
        ...
        foreach (var (gpu, best, all) in winners)
        {
            ...
            string snArg = best.Cfg.SearchN != 131072 ? $" & set \"ARC_SEARCH_N={best.Cfg.SearchN}\"" : "";
            Console.WriteLine(
                $"     set \"ARC_TGEMM_NB={best.Cfg.Nb}\" & set \"ARC_TGEMM_MB={best.Cfg.Mb}\" & set \"ARC_SEARCH_M={best.Cfg.SearchM}\"{snArg}");
            Console.WriteLine(bar);
        }
    }
    ...
    private static List<WorkerOrchestrator.GpuInfo> EnumerateGpus()
    {
        ...
        for (int ord = 0; ord < n; ord++)
        {
            ...
            // Skip integrated GPUs unless --igpu — autotuning an iGPU wastes the
            // sweep and would cache a useless profile.
            if (!GpuSelection.IgpuEnabled && GpuSelection.IsIntegratedByName(name)) continue;
    ...
    internal static class TuneCache
    {
        private const string Header =
            "# arc-miner autotune cache v2 | sku|nb|mb|search_m|search_n|tmads|version|utc";
        ...
        public static void Write(string path, IEnumerable<(string Sku, int Nb, int Mb, int SearchM, int SearchN, double Tmads)> rows)
        {
            ...
            foreach (var (sku, nb, mb, searchM, searchN, tmads) in rows)
                byKey[sku] = $"{sku}|{nb}|{mb}|{searchM}|{searchN}|{tmads:F1}|{ver}|{utc}";
        ...
        /// <summary>Look up a cached (NB, MB, SEARCH_M, SEARCH_N) for a GPU model name.
        /// Returns null if absent. Used by the mining path to auto-apply.</summary>
        public static (int Nb, int Mb, int SearchM, int SearchN)? Lookup(string path, string sku)
        {
            ...
            foreach (var line in File.ReadAllLines(path))
            {
                ...
                {
                    int sn = 131072;
                    if (f.Length >= 6 && int.TryParse(f[4], out var parsedSn) && parsedSn > 0)
                        sn = parsedSn;
                    return (nb, mb, sm, sn);
                }
            }
