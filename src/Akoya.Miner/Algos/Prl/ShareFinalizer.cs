...
internal sealed class ShareFinalizer : IAsyncDisposable
{
    ...
    private void Finalize(SharePayload p)
    {
                ...
                share.ClaimedHash.Span, p.ClaimedDifficultyNbits, p.MiningConfig))
        {
            ...
            // NOTHING to do with target movement.
            //
            // This line used to print claimedHash[0..8] and it could not answer
            // the question it existed to answer. The hash is compared as a
            // LITTLE-ENDIAN 256-bit integer, so bytes 0..7 are its LOW end —
            // the least significant bytes, the ones that say nothing about
            // whether it cleared a target near 2^223. Whether a skip was a near
            // miss or garbage lives in bytes 24..31, which were the ones not
            // printed. Print the whole hash and the target it lost to.
            ReportSkip(p, share);
            return;
        }
        ...
        // Record which GPU this share came from BEFORE handing it to the
        // session — WorkerOrchestrator.OnShareResult dequeues FIFO to credit
        // the pool's verdict to the right per-GPU counter (ack carries no id).
        WorkerOrchestrator.PendingShareGpu.Enqueue(p.GpuIndex);
    ...
    /// <summary>
    /// Explain a pre-submit skip with the numbers the comparison actually used.
    ///
    /// The hash is compared as a LITTLE-ENDIAN 256-bit integer, so its bytes
    /// 0..7 are the LEAST significant ones and say nothing about whether it
    /// cleared a target near 2^223 — an earlier version of this line printed
    /// exactly those and so could not answer the question it existed to answer.
    /// Print the whole hash, the target it lost to, and the gap in bits: one or
    /// two bits is a near miss (a target that moved, or rounding); ~30 bits
    /// means the hash is unrelated to what the GPU found, i.e. the host and the
    /// device have diverged somewhere.
    /// </summary>
    private void ReportSkip(SharePayload p, ShareSubmission share)
    {
        var hash = share.ClaimedHash.Span;
        var hashInt = new BigInteger(hash, isUnsigned: true, isBigEndian: false);
        var target = SigmaContext.NbitsToTarget(p.ClaimedDifficultyNbits)
                   * p.MiningConfig.DifficultyAdjustmentFactor();
        if (target > MaxTarget) target = MaxTarget;

        double overBits = target > BigInteger.Zero && hashInt > BigInteger.Zero
            ? BigInteger.Log(hashInt, 2) - BigInteger.Log(target, 2)
            : double.NaN;

        _log.LogWarning(
            "worker[{Gpu}]: share skipped — claimedHash > target by {Over:F1} bits "
            + "(nbits=0x{Nbits:X8} DAF={Daf} salted={Salted} tile=({Tr},{Tc}) "
            + "sigma[0..8]={Sigma} jobKey[0..8]={Job})\n"
            + "    claimedHash(LE) = {Claimed}\n"
            + "    target          = {Target}",
            p.GpuIndex, overBits, p.ClaimedDifficultyNbits,
            p.MiningConfig.DifficultyAdjustmentFactor(), p.SaltedSeeds,
            p.TileRow, p.TileCol,
            Convert.ToHexString(p.Sigma.AsSpan(0, Math.Min(8, p.Sigma.Length))),
            Convert.ToHexString(p.JobKey.AsSpan(0, Math.Min(8, p.JobKey.Length))),
            Convert.ToHexString(hash),
            target.ToString("X64"));
    }

    // Per-share diagnostic trace, opt-in via ARC_SHARE_TRACE=1. Prints the
    ...
    private static readonly bool s_shareTrace =
        (Akoya.Crypto.MinerEnv.Get("ARC_SHARE_TRACE") ?? "") == "1";
