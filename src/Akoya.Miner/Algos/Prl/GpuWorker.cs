...
internal sealed partial class GpuWorker : IAsyncDisposable, ILivenessTarget
{
    ...
    // still blocks until the GPU is actually done). ARC_SPIN_SYNC=1 reverts to
    // pure spin; ARC_SYNC_SPIN_MS sets the spin-tail margin (default 5ms).
    private static readonly bool s_spinSync =
        Environment.GetEnvironmentVariable("ARC_SPIN_SYNC") == "1";
    private static readonly int s_syncSpinMs =
        int.TryParse(Environment.GetEnvironmentVariable("ARC_SYNC_SPIN_MS"), out var m) && m >= 0 ? m : 5;
    ...
    private sealed class WorkerState
    {
        ...
        // Seed derivation this σ was installed under (salted-seed fork, PR #280).
        // Recorded rather than re-read at share-build time: the device bakes the
        // flag in when a batch is ENQUEUED, so a share must be rebuilt with the
        // value that was in force for its own σ, not whatever the process has
        // moved on to. See InstallSigma.
        public bool   InstalledSaltedSeeds;
    ...
    private void Loop(CancellationToken ct)
    {
        ...
        try
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                //
                //     Emergency switch: if ARC_DISABLE_PONG=1, force pongSize=0
    ...
    private void RecordSigmaRotation(SigmaContext ctx, SigmaRotationTiming timing)
    {
        ...
        // Internal rotation timing — debug-only so the default log stays readable
        // (fires on every job change). Enable with debug/verbose logging.
        _log.LogDebug(
    ...
    private static void InstallResidentBState(WorkerHalf scratchHalf, SigmaContext ctx, WorkerState s)
    {
        ...
        // A is sm-sized on SYCL — regen+hash via the fused kernel (writes only the
        // sm rows to A, hashes full A in-register; same AHash as the two-pass path).
        Check("tensor_hash_fused A seed", PearlGemm.PearlGemmNative.TensorHashFusedLeafCvs(
            a.A.Handle, THROWAWAY_A_SEED_LO, s.SigmaSeed, bA, a.ABufferBytes,
            bs.Key.Handle, a.Roots.Handle, a.AHash.Handle, nint.Zero, scratchHalf.Stream.Handle));
    ...
    private static void InstallSigmaHalf(WorkerHalf half, SigmaContext ctx, WorkerState s)
    {
        ...
        unsafe
        {
            var wp = new PearlGemm.PearlGemmNative.WorkspaceParams
            {
                ...
                SyclKSub    = s.SyclKSub,
                // Latched by InstallSigma before it reaches either half, so
                // both streams of this σ agree with each other and with the
                // SharePayload the host will rebuild from.
                SaltedSeeds = s.InstalledSaltedSeeds ? 1 : 0,
    ...
    private SigmaRotationTimingBuilder InstallSigma(WorkerState s, SigmaContext ctx, double oldBatchDrainMs)
    {
        ...
        _log.LogInformation(
            // Shape/config census. The declared MiningConfiguration is what the
            // target scaling and the verifier's row re-derivation are built on;
            // the buffers are what the kernel actually computes. If those two
            // disagree the miner mines one shape and proves another, and every
            // share dies. They are logged together, once per σ, because reading
            // either alone tells you nothing.
            "worker[{Gpu}]: σ install height={H} nbits=0x{Nbits:X8} job={Job} (rotate={Rot}) "
            + "cfg[commonDim={CD} rank={RK} rows.size={RS} cols.size={CS} dot={Dot} DAF={Daf}] "
            + "buf[M={BM} N={BN} K={BK} R={BR}]",
            _gpuIndex, ctx.BlockHeight, ctx.TargetNbits, ctx.JobId, !sameAsInstalled,
            s.MiningConfig.CommonDim, s.MiningConfig.Rank,
            s.MiningConfig.RowsPattern.Size, s.MiningConfig.ColsPattern.Size,
            s.MiningConfig.DotProductLength(), s.MiningConfig.DifficultyAdjustmentFactor(),
            s.Ping.Buffers.M, s.Ping.Buffers.N, s.Ping.Buffers.K, s.Ping.Buffers.R);
        // Crypto prefixes are debug-only (verify/diagnostic detail, not needed in
        // the default scroll).
        _log.LogDebug(
            "worker[{Gpu}]: σ install detail sigma={SigmaPrefix} jobKey={JobKeyPrefix} bSeed={BSeedPrefix} auditK={AuditK}",
            _gpuIndex,
            Convert.ToHexString(ctx.Sigma.AsSpan(0, Math.Min(8, ctx.Sigma.Length))),
        ...
        // Salted-seed fork: flip the derivation HERE, on the mining thread, with
        // both streams drained — not on the job thread that publishes to the bus.
        // The device resolves the flag when a batch is enqueued and the host
        // re-derives the seeds when it builds the share; if those two reads
        // straddle the flip, the miner searches one noise field and proves
        // another, and every share of that batch dies as "claimedHash >
        // liveTarget" before it ever reaches the pool. Doing it at σ install,
        // between the drain and the next QueueBatch, means no batch can span the
        // change. Both early returns above are same-σ (hence same height), so
        // they cannot be skipping a flip.
        //
        // The value is then latched onto WorkerState and travels two ways: into
        // this σ's WorkspaceParams (so the kernel reads it per-σ rather than
        // from a process global that another GPU's worker could move mid-batch)
        // and onto every SharePayload (so the host rebuild uses the same one).
        SaltedSeedFork.Apply(ctx.BlockHeight, _log);
        s.InstalledSaltedSeeds = SaltedSeedFork.Active;
    ...
    private void HandleTrigger(WorkerHalf half, WorkerState s, int winK, ulong winSeed, CancellationToken ct)
    {
        ...
        // (The resident A buffer is sm-sized, so A is regenerated INSIDE the
        // proof path below: the fast path's fused leaf-CV call rebuilds it
        // in-register; the rare fallback regens full A into FullAScratch.)
        ...
        {
            // The canonical full-A path is the DEFAULT now, not an anomaly, so
            // taking it is not a warning — it would fire on every trigger and
            // train operators to ignore the one log level that matters here.
            // A genuine shape-driven blocker still warns.
            _log.LogWarning(
        ...
        {
            // SYCL: regen A for the winning seed via the FUSED kernel — writes the
            // sm search rows to the (sm-sized) A buffer and produces the full-A
            // leaf-CV table + Merkle root in-register, with NO full-A buffer. The
            // opened rows (< sm) are in the persisted prefix, so the selected-row
            // D2H below reads them straight from A.
            Check("tensor_hash_fused A leaf_cvs", PearlGemm.PearlGemmNative.TensorHashFusedLeafCvs(
                b.A.Handle, winSeed, s.SigmaSeed, bA, b.ABufferBytes,
                s.BState.Key.Handle, b.Roots.Handle, b.AHash.Handle, b.ALeafCvs.Handle,
                half.Stream.Handle));
        ...
        else
        {
            ...
            // The resident A is sm-sized — regen full A into the lazy full-size
            // scratch first (allocated on first use; not hit on the Arc fast path).
            Check("lcg_int7 A regen (fallback)", PearlGemm.PearlGemmNative.LcgInt7Fill(
                b.FullAScratch.Handle, bA, winSeed, s.SigmaSeed, half.Stream.Handle));
            var aSrc = b.FullAScratch;
            CudaDriver.Check(
                CudaDriver.MemcpyDtoHAsync(b.HostAPtr, aSrc, (nuint)bA, half.Stream),
                "D2H A async");
        ...
        var payload = new SharePayload(
            ...
            SaltedSeeds:            s.InstalledSaltedSeeds,
    ...
    private void MaybeLogStats(WorkerState s)
    {
        ...
        if (!Dashboard.Active)
            _log.LogInformation(
                ...
                _gpuIndex, DisplayFormat.HashRate(hashesPerSec), FormatDifficulty(s.InstalledTargetNbits), iterMs, ips,
        ...
        Metrics.SetDiff(_gpuIndex, FormatDifficulty(s.InstalledTargetNbits));
    ...
    /// <summary>
    /// Is the compact leaf-CV A-proof path allowed? DEFAULT: NO.
    ///
    /// The fast path reconstructs A's Merkle root from the GPU's leaf-CV table
    /// (<see cref="MerkleProofFromLeafCvs"/>) instead of hashing A itself, and
    /// that reconstruction DISAGREES with the root the GPU's own tensor_hash
    /// produces. Measured on a B580, same σ, host and device traced together:
    ///
    ///   jobKey  F7DA8E61…  ==  F7DA8E61…   (agree)
    ///   B_root  053FBC8C…  ==  053FBC8C…   (agree — proves the CPU and GPU
    ///                                       Merkle implementations agree in
    ///                                       general, and that jobKey/m/n/salted
    ///                                       reach both sides identically)
    ///   A_root  CAAF3B38…  !=  A50E158B…   ← the bug
    ///
    /// A_root feeds a_seed feeds the E_A noise feeds the jackpot, so the host
    /// proves a share against a different noise field than the GPU searched:
    /// the claimed hash lands ~30 bits over target and EVERY share is dropped
    /// pre-submit. 100% loss, silent, with the GPU still pegged and the
    /// hashrate reading correct.
    ///
    /// The fallback D2Hs full A and uses the Rust-canonical
    /// <c>MerkleRootAndProof.Compute</c> — the same implementation the matching
    /// B_root above proves correct — so it produces both the root AND the
    /// inclusion proof consistently. It costs a full-A D2H and lazily allocates
    /// M×K of VRAM plus M×K of pinned host RAM per half (512 MiB each at
    /// canonical M=131072 K=4096). That is real memory, but it is only touched
    /// on a TRIGGER, and triggers are minutes apart, so steady-state hashrate is
    /// unaffected.
    ///
    /// UPDATE — the leaf-CV path was NOT the bug and is re-enabled by default.
    /// The real fault was install_B deriving the B-side seed through a wrapper
    /// that hardcoded (m=0, n=0, salted=false), so the whole B noise field was
    /// legacy while the host proved V3. Forcing the canonical full-A path was
    /// tested against that and changed nothing, while costing ~512 MiB of VRAM
    /// and ~512 MiB of pinned host RAM per half. Default restored.
    ///
    /// Set ARC_PRL_FAST_A_PROOF=0 to force the canonical full-A path — it is a
    /// genuine, slower, memory-hungry cross-check of the leaf-CV reconstruction,
    /// which is worth having when a share stops verifying.
    /// </summary>
    private static readonly bool s_fastAProofAllowed =
        (Akoya.Crypto.MinerEnv.Get("ARC_PRL_FAST_A_PROOF") ?? "") is not ("0" or "false");

    private static string? FastAProofPathBlocker(WorkerBuffers b, uint[] rowIndices, long aBytes)
    {
        if (!s_fastAProofAllowed) return "ARC_PRL_FAST_A_PROOF=0 — canonical full-A path forced";
        ...
        unsafe
        {
            fixed (byte* pSeed = seedBuffer)
            {
                var p = new PearlGemm.PearlGemmNative.InstallBParams
                {
                    ...
                    // The B-side noise is derived from this ONCE per σ and is
                    // never regenerated per-iter, so it must carry the same
                    // derivation the host will prove against.
                    SaltedSeeds = SaltedSeedFork.Active ? 1 : 0,
        ...
        try
        {
            ...
            foreach (bool bSeedChanged in new[] { false, true })
            {
                ...
                for (int run = 1; run <= repeat && !ct.IsCancellationRequested; run++)
                {
                    ...
                    {
                        Check("tensor_hash_fused A seed", PearlGemm.PearlGemmNative.TensorHashFusedLeafCvs(
                            ping.Buffers.A.Handle, THROWAWAY_A_SEED_LO, sigmaSeed, bA, ping.Buffers.ABufferBytes,
                            bState.Key.Handle, ping.Buffers.Roots.Handle, ping.Buffers.AHash.Handle, nint.Zero, ping.Stream.Handle));
                    });
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            log.LogInformation(
                ...
                deviceOrdinal, iters, sec, ips, tmads, tilesPerSec, DisplayFormat.HashRate(hashesPerSec));
    ...
    // the ARC_SEARCH_N logic in native/pearl-gemm/csrc/sycl/
    ...
    {
        int sm = EnvInt("ARC_SEARCH_M", 4096);
        if (sm > m) sm = m;
        sm = (sm / 16) * 16;
        if (sm < 16) sm = 16;
        return sm;
    ...
    /// <summary>Mirror of the ARC_SEARCH_N logic in pearl_capi_iter().</summary>
    internal static int SyclSearchN(int n)
    {
        int sn = EnvInt("ARC_SEARCH_N", 131072);
        if (sn > n) sn = n;
        sn = (sn / 64) * 64;
        if (sn < 64) sn = 64;
        if (sn > n) sn = n;
        ...
        int bn)
    {
        ...
        unsafe
        {
            var wp = new PearlGemm.PearlGemmNative.WorkspaceParams
            {
                ...
                SigmaSeed = sigmaSeed,
                SyclKSub = syclKSub,
                // Benchmark workspace: the σ here is throwaway and its output
                // is never submitted, so the derivation only has to be a valid
                // one. Track the process default rather than pinning legacy, so
                // a post-fork benchmark measures the kernel the rig will
                // actually run.
                SaltedSeeds = SaltedSeedFork.Active ? 1 : 0,
        ...
        ulong sigmaSeed)
    {
        ...
        H2D(bState.Key, jobKey);
        Check("tensor_hash_fused A seed", PearlGemm.PearlGemmNative.TensorHashFusedLeafCvs(
            a.A.Handle, THROWAWAY_A_SEED_LO, sigmaSeed, bA, a.ABufferBytes,
            bState.Key.Handle, a.Roots.Handle, a.AHash.Handle, nint.Zero, scratchHalf.Stream.Handle));
