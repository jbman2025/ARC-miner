...
internal static class ShareBuilder
{
        ...
        bool collectTimings = true)
    {
        ...
        stageStart = TimingStart(collectTimings);
        var (bNoiseSeed, aNoiseSeed) = CommitmentHasher.DeriveNoiseSeeds(jobKey, hashA, hashB, m, n, saltedSeeds);
        SeedTrace.Dump(jobKey, hashA, hashB, m, n, saltedSeeds, aNoiseSeed, bNoiseSeed);
    ...
    public static ShareSubmission Build(
        ...
        uint claimedDifficultyNbits,
        byte[] hashB,
        bool saltedSeeds = false)
        => Build(sigma, configBytes, jobKey, aBytes, bBytes, aRowIndices, bColIndices,
                 tileRow, tileCol, m, n, k, r, claimedDifficultyNbits, hashB, saltedSeeds, out _);
