...
internal sealed class WorkerBuffers : IDisposable
{
    ...
    private CUdeviceptr _fullAScratch;   // lazy full-M*K device buffer for the rare full-A proof fallback
    ...
    /// <summary>Byte size of the resident <see cref="A"/> buffer — sm*K (search
    /// window) on SYCL, full M*K otherwise. The trigger-path fused regen persists
    /// exactly this many bytes (the sm rows the opened tile lives in).</summary>
    public long ABufferBytes { get; }
    ...
    public CUdeviceptr A         { get; }   // SYCL: sized to sm*K (search window); CUDA/ROCm: full M*K

    /// <summary>Lazy full-M*K device buffer for the RARE full-A proof fallback
    /// (non-1024-aligned widths / unexpected open patterns — not hit on the
    /// production Arc fast path, where the fused trigger regen needs no full-A
    /// buffer). Allocated only if <c>FastAProofPathBlocker</c> fires.</summary>
    public CUdeviceptr FullAScratch
    {
        get
        {
            if (_fullAScratch.Handle == nint.Zero)
                _fullAScratch = Alloc((long)M * K);
            return _fullAScratch;
        }
    }
    ...
    public WorkerBuffers(int m, int n, int k, int noiseRank, int matmulsPerPoll)
    {
        ...
        // Both A and ApEA only ever hold the SEARCH-window rows: the fused
        // iter writes/reads A[0..sm) and ApEA[0..sm) (add_i8 over sm*k, tgemm over
        // the sm×sn tile window) — the full M*K alloc (512 MiB EACH per half at
        // canonical) is dead. Full A is needed only on a share trigger: the fused
        // regen (TensorHashFusedLeafCvs) reproduces it in-register (no buffer), and
        // the rare full-A proof fallback uses the lazy FullAScratch. Size A and ApEA
        // to the kernel's search-M, queried NATIVELY so it matches compute_search_m
        // / the UCRT getenv the kernel reads (can't drift even if autotune sets
        // SEARCH_M natively). The iter clamps sm to the workspace sm_cap (also
        // compute_search_m(M)), so this is always ≥ what it writes.
        long smRows = PearlGemmNative.SearchM(M);
        if (smRows <= 0 || smRows > M) smRows = M;   // defensive: fall back to full size
        long bASearch = smRows * K;
        ...
        // AxEBL projection is bypassed on Arc (pearl_capi_noisy_gemm is a stub and
        // pearl_capi_iter ignores the pointer), so an M*R*2 buffer (~67 MiB/half,
        // ~134 MiB/worker) would be dead VRAM. Pass null — useful headroom on
        // 8 GB A750/A770.
        AxEBLFp16 = default;
        ApEA      = AllocZero(bASearch);
    ...
    public void Dispose()
    {
        ...
        TryFree(A);          TryFree(_fullAScratch);
