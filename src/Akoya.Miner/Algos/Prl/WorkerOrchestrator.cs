...
{
    /// <summary>FIFO of the GPU index for each submitted share, enqueued by
    /// ShareFinalizer at SubmitAsync and dequeued in OnShareResult — the pool
    /// ack carries no GPU id, and the single-session/single-consumer pipeline
    /// keeps ack order == submit order. Multi-GPU rigs credited per-GPU stats
    /// to GPU 0 without this.</summary>
    internal static readonly System.Collections.Concurrent.ConcurrentQueue<int> PendingShareGpu = new();
        ...
        CancellationTokenSource tripCts)
    {
        try
        {
            ...
            bus.Publish(ctx);
            Metrics.SetBlockHeight(ctx.BlockHeight);
            // Same height, recorded against Pearl specifically — SetBlockHeight is
            // written by every GPU algo, so it cannot be measured against Pearl's
            // fork schedule.
            Metrics.SetPrlBlockHeight(ctx.BlockHeight);
            // Salted-seed hardfork: NOT applied here. It used to be, and that is
            // a race — this runs on the job thread, while a worker is mid-batch
            // on a σ that was enqueued under the old derivation. The device bakes
            // the flag in at enqueue time and the host re-reads it when it builds
            // the share, so a flip landing between those two reads makes the
            // miner search one noise field and prove another: the whole batch
            // dies locally as "claimedHash > liveTarget". GpuWorker.InstallSigma
            // owns the flip instead, where both streams are drained and no batch
            // can span it.
            //
            // Remember it for the NEXT process: on stratum this is the only
            // place the height is ever known. Feeds the dashboard's fork counter
            // across a cold start, nothing else.
            PrlHeightStore.Persist(ctx.BlockHeight);
    ...
    /// <summary>Default run of consecutive stale / job-not-found share
    /// rejects we tolerate before forcing a reconnect. A pool that silently
    /// stops sending new <c>mining.notify</c> (job-feed TTL, dropped
    /// subscription) leaves us mining the last σ: every share is
    /// mathematically valid but submitted against a job the pool has expired,
    /// so it returns reason="job not found / stale". The stratum idle
    /// watchdog can't see this — each reject IS an inbound line, so it counts
    /// as liveness and never trips. A run of these is the unambiguous "the
    /// job feed is dead" signal; the only recovery is a fresh session
    /// (re-authorize → fresh job feed). A healthy pool ACCEPTS shares, so
    /// this never false-trips. Env override: ARC_STALE_REJECT_RECONNECT
    /// (0 disables). Exposed internal so tests can assert against it.</summary>
    internal const int DefaultStaleRejectReconnect = 5;

    /// <summary>Default job-staleness budget (seconds): force a reconnect if
    /// no NEW job (<c>mining.notify</c> / JobAssignment) has arrived within
    /// this window, regardless of other inbound traffic. Backstops the
    /// stale-reject guard for the "feed died and we have no shares to submit"
    /// case. 600s (10 min) is far longer than any healthy notify gap (pools
    /// push at least per-block, usually far more often) but well under the
    /// observed ~1h dead-feed failure — so a quiet-but-healthy pool is never
    /// reconnect-thrashed. Env override: ARC_JOB_STALE_RECONNECT_SEC
    /// (0 disables). Exposed internal so tests can assert against it.</summary>
    internal const int DefaultJobStaleReconnectSec = 600;

    /// <summary>True when a pool's share-reject message indicates the job is
    /// stale / no longer known (the share was valid but submitted against an
    /// expired job). Matches the common phrasings across stratum and V2 pools
    /// — the stratum reject surfaces the raw error JSON, e.g.
    /// <c>{"code":21,"msg":"job not found / stale"}</c>.</summary>
    internal static bool IsStaleJobReject(string? message)
    {
        if (string.IsNullOrEmpty(message)) return false;
        return message.Contains("stale", StringComparison.OrdinalIgnoreCase)
            || message.Contains("job not found", StringComparison.OrdinalIgnoreCase)
            || message.Contains("job_not_found", StringComparison.OrdinalIgnoreCase)
            || message.Contains("unknown job", StringComparison.OrdinalIgnoreCase);
    }
    ...
    public async Task RunAsync(CancellationToken ct)
    {
        ...
        bool isStratum = _opts.Pool.Host.Contains("herominers")
                      ...
                      || Environment.GetEnvironmentVariable("ARC_POOL_STRATUM") == "true"
                      || Environment.GetEnvironmentVariable("ARC_POOL_STRATUM") == "1";
        ...
        //     k=4096, noise_rank=128. M/N are NOT tunable here (committed), so the
        ...
        //
        // ARC_MAINNET_M / ARC_MAINNET_N are an escape hatch for the stratum
        // path; explicit ARC_MINE_* overrides bypass this whole block.
        if (!_opts.Mine.ShapeOverridePresent)
        {
            if (isStratum)
            {
                int mainnetM = ParseEnvInt("ARC_MAINNET_M", 131072);
                int mainnetN = ParseEnvInt("ARC_MAINNET_N", 131072);
                // Rank is pinned at the consensus floor. The rank-penalty softfork
                // (pearl PR #275, mainnet 96,251) scales the jackpot bound by
                // 128/rank and rejects anything below 128, so 128 is both the
                // minimum and the only rank that earns full reward. Mainnet is
                // long past activation, so the pre-fork 256 profile — and the
                // height-sniffing that used to choose between them — is gone.
                const int rank = 128;
                mineProfile = mineProfile with { M = mainnetM, N = mainnetN, K = 4096, NoiseRank = rank };
                mineProfileName = "stratum-mainnet";
                _log.LogInformation("mine-profile: noise_rank={Rank} (consensus floor)", rank);
        ...
        {
            // Explicit ARC_MINE_M/N/K/NOISE_RANK override — caller takes
        ...
        // 4a. Hashrate benchmark — sample each GPU for ARC_BENCH_DURATION_SEC
        ...
        //     mpp 1). Mandatory — there is no ARC_BENCH_DISABLE.
        //
        ...
        // Consecutive stale / job-not-found share rejects (see
        // DefaultStaleRejectReconnect + IsStaleJobReject). Reset on any
        // accepted share or any non-stale reject; a sustained run forces a
        // reconnect. Local to this session attempt, so it naturally resets on
        // every reconnect. 0 (env) disables the guard entirely.
        int consecutiveStaleRejects = 0;
        int staleRejectTrip =
            int.TryParse(Akoya.Crypto.MinerEnv.Get("ARC_STALE_REJECT_RECONNECT"), out var srt) && srt >= 0
                ? srt : DefaultStaleRejectReconnect;

        // Job-staleness watchdog state. Backstops the stale-reject guard: that
        // one needs rejected shares to count, but a pool can stop sending new
        // mining.notify AND leave us finding no shares to even submit (so no
        // rejects arrive). Track the TickCount64 of the last NEW job; a
        // background loop trips reconnect if no fresh work arrives within the
        // budget — regardless of other inbound traffic (pongs, vardiff, the
        // stratum line-idle watchdog all stay "live" while the job feed is
        // dead). One-element array so the watchdog task can volatile-read it
        // across threads. Seeded to "now": we've just authorized, so the first
        // job is expected shortly; if none ever comes we still recover.
        // Budget via ARC_JOB_STALE_RECONNECT_SEC (0 disables); the default is
        // generous so a legitimately quiet pool is never reconnect-thrashed.
        var lastJobTickBox = new long[] { Environment.TickCount64 };
        int jobStaleSec =
            int.TryParse(Akoya.Crypto.MinerEnv.Get("ARC_JOB_STALE_RECONNECT_SEC"), out var jss) && jss >= 0
                ? jss : DefaultJobStaleReconnectSec;
        ...
        var callbacks = new MiningSessionCallbacks
        {
            OnJob = job =>
            {
                ...
                // Fresh work arrived — refresh the job-staleness clock. Done
                // for any notify the pool sends (a malformed run is handled by
                // the parse-failure threshold above); the signal we want here
                // is simply "the job feed is still alive".
                Volatile.Write(ref lastJobTickBox[0], Environment.TickCount64);
                return ValueTask.CompletedTask;
            },
            OnShareResult = result =>
            {
                // The pool ack carries no GPU id — correlate FIFO with the
                // submit order (single stratum session, single ShareFinalizer
                // consumer ⇒ acks arrive in submit order). Falls back to GPU 0
                // if the queue ever skews (stats-only; totals stay correct).
                if (!PendingShareGpu.TryDequeue(out var shareGpu)) shareGpu = 0;
                if (result.Accepted) Metrics.IncShareAccepted(shareGpu);
                else                 Metrics.IncShareRejected(shareGpu);
                ...
                else
                {
                    ...
                    // Stale/job-not-found run → the pool's job feed has gone
                    // dead under us (we're mining a σ whose job the pool has
                    // expired). Nothing else trips on this — the stratum idle
                    // watchdog sees each reject as a live inbound line. Force a
                    // reconnect once the run crosses the threshold so we
                    // re-authorize onto a fresh job feed instead of mining a
                    // dead job indefinitely. A non-stale reject breaks the run.
                    // Count ANY consecutive reject, not just stale/job-not-found.
                    // Observed 2026-07-02 on unmineable: the pool's notify job-id
                    // field froze (headers kept rotating) → every share came back
                    // {"code":24,"msg":"Invalid Share"} until the pool dropped the
                    // stream 4 minutes later. That message never matched the stale
                    // patterns, so the old stale-only guard let the session bleed
                    // shares into a dead job feed. A run of N rejects with zero
                    // accepts in between is pathological whatever the reason, and
                    // a reconnect (fresh subscribe → fresh job ids) is the fix for
                    // the whole class. Stale rejects keep their specific reason tag.
                    if (staleRejectTrip > 0)
                    {
                        var n = ++consecutiveStaleRejects;
                        if (n >= staleRejectTrip)
                        {
                            var stale = IsStaleJobReject(result.Message);
                            _log.LogWarning(
                                "orchestrator: {N} consecutive share rejects (last reason: {Reason}) — pool session is wedged, forcing reconnect",
                                n, result.Message ?? "");
                            TripWorkers(stale ? "stale_job_rejects" : "consecutive_share_rejects");
                        }
                    }
                }
                return ValueTask.CompletedTask;
        ...
        // 7b. Job-staleness watchdog. Independent of the stream/worker
        //     watchdogs: it trips when the pool stops feeding new work while
        //     the connection is otherwise alive — the exact failure the
        //     stratum line-idle watchdog can't see, because share-reject
        //     replies keep it "live". Started here; cancelled in the finally
        //     so it never trips during teardown.
        using var jobStaleCts = CancellationTokenSource.CreateLinkedTokenSource(ct);
        var jobStaleTask = jobStaleSec > 0
            ? Task.Run(() => JobStalenessLoop(
                lastJobTickBox,
                TimeSpan.FromSeconds(jobStaleSec),
                TripWorkers,
                orchLog,
                jobStaleCts.Token), jobStaleCts.Token)
            : Task.CompletedTask;
        ...
        {
            // Stop the job-staleness watchdog first so it can't trip while we
            // tear down (no new jobs arrive during teardown by design).
            jobStaleCts.Cancel();
            try { await jobStaleTask.ConfigureAwait(false); } catch { /* shutdown */ }
    ...
    /// <summary>Background loop that forces a reconnect when no NEW job has
    /// arrived within <paramref name="budget"/>. <paramref name="lastJobTickBox"/>[0]
    /// is the <see cref="Environment.TickCount64"/> of the last JobAssignment,
    /// volatile-written from the OnJob callback. Tripping here calls
    /// <paramref name="trip"/> (→ workerTripCts.Cancel), which unblocks
    /// RunStreamAsync and drives Program.cs's reconnect — a fresh authorize
    /// re-opens the job feed. See <see cref="DefaultJobStaleReconnectSec"/>.</summary>
    private static async Task JobStalenessLoop(
        long[] lastJobTickBox,
        TimeSpan budget,
        Action<string, Exception?> trip,
        ILogger log,
        CancellationToken ct)
    {
        // Poll at ¼ of the budget (clamped 1–30s) so we overshoot the
        // threshold by at most ~budget/4 without busy-looping a long budget.
        var tickMs = Math.Max(1000, Math.Min(30000, budget.TotalMilliseconds / 4));
        ...
        try
        {
            while (!ct.IsCancellationRequested)
            {
                ...
                var idle = Environment.TickCount64 - Volatile.Read(ref lastJobTickBox[0]);
                if (idle > budget.TotalMilliseconds)
                {
                    log.LogWarning(
                        "orchestrator: no new job for {Idle:F0}ms (> {Budget:F0}ms) — pool job feed is stale, forcing reconnect",
                        idle, budget.TotalMilliseconds);
                    trip("job_feed_stale", null);
                    return;
                }
            }
        }
    ...
    private void ApplyTunedProfile(List<GpuInfo> gpus)
    {
        ...
        try
        {
            ...
            ApplyKnob("ARC_TGEMM_NB", t.Nb, applied);
            ApplyKnob("ARC_TGEMM_MB", t.Mb, applied);
            ApplyKnob("ARC_SEARCH_M", t.SearchM, applied);
            if (t.SearchN > 0 && t.SearchN != 131072)
                ApplyKnob("ARC_SEARCH_N", t.SearchN, applied);
    ...
    // instead of relying on ARC_ROWS_PATTERN/ARC_COLS_PATTERN being set at launch.
    ...
    /// reads ARC_ROWS_PATTERN/ARC_COLS_PATTERN; here we set the process default for profiles whose
    ...
    private static void ApplyHashTileDefaults(HashTile tile)
    {
        ...
        if (string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("ARC_ROWS_PATTERN")))
            Environment.SetEnvironmentVariable("ARC_ROWS_PATTERN", idx);
        if (string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("ARC_COLS_PATTERN")))
            Environment.SetEnvironmentVariable("ARC_COLS_PATTERN", idx);
    }
    ...
    private List<GpuInfo> EnumerateGpus()
    {
        ...
        // Only auto-skip integrated GPUs when the operator asked for "all";
        // an explicit index list is always honored (they picked it on purpose).
        bool selectingAll = string.Equals(_opts.Gpus.IndicesRaw, "all", StringComparison.OrdinalIgnoreCase);
        bool filterIgpu = selectingAll && !GpuSelection.IgpuEnabled;
        int skippedIgpu = 0;
        ...
        foreach (var ord in wanted)
        {
            ...
            if (filterIgpu && GpuSelection.IsIntegratedByName(name))
            {
                _log.LogInformation("gpu[{Ord}] \"{Name}\" is integrated — skipping (pass --igpu to include it)", ord, name);
                skippedIgpu++;
                continue;
            }
            ...
            // PCI address is how the sysfs hwmon sensors are keyed, so record it
            // against this slot — note the slot is result.Count, not ord, because
            // filtered-out devices (iGPUs) make the two diverge. Best-effort: an
            // older shim build returns a placeholder for every device, which
            // GpuIdentity discards rather than mislabelling cards.
            var pci = GpuIdentity.TryGetPciAddress(ord);
            if (!string.IsNullOrEmpty(pci) && pci != GpuIdentity.PlaceholderPci)
                Metrics.SetGpuPciAddress(result.Count, pci);

            result.Add(new GpuInfo(ord, name, synthetic, major, minor));
        }

        if (result.Count == 0 && skippedIgpu > 0)
            throw new InvalidOperationException(
                $"only integrated GPU(s) found ({skippedIgpu} skipped) — pass --igpu to mine on them.");
        return result;
    }
    ...
    private static List<int> ParseGpuSelection(string raw, int deviceCount)
    {
        ...
        foreach (var tok in raw.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
        {
            if (!int.TryParse(tok, out var i))
                throw new FormatException($"ARC_GPU_INDICES: bad token '{tok}'");
