...
// any v1-era ARC_* var that has no V2 effect.
//
...
internal static class EnvVarBindings
{
    ...
    {
        ("ARC_GATEWAY_HOST",                         "no solo/gateway mode in V2"),
        ("ARC_GATEWAY_PORT",                         "no solo/gateway mode in V2"),
        ("ARC_POOL_FIRST_JOB_TIMEOUT_SEC",           "gRPC server-push delivers job; no race"),
        ("ARC_POOL_FIRST_JOB_GIVE_UP_AFTER",         "gRPC server-push delivers job; no race"),
        ("ARC_POOL_RECONNECT_BASE_SEC",              "gRPC channel handles reconnect backoff"),
        ("ARC_POOL_RECONNECT_CAP_SEC",               "gRPC channel handles reconnect backoff"),
        ("ARC_POOL_RECONNECT_INITIAL_JITTER_SEC",    "gRPC channel handles reconnect backoff"),
        ("ARC_POOL_RECONNECT_MAX_WAIT_SEC",          "use ReconnectHint.wait_seconds from server"),
        ("ARC_POOL_RECONNECT_MIN_INTERVAL_SEC",      "server-side concern in V2"),
        ("ARC_POOL_SHARE_STARVATION_SEC",            "v1 protocol workaround; not needed in V2"),
        ("ARC_POOL_SHARE_STARVATION_COOLDOWN_SEC",   "v1 protocol workaround; not needed in V2"),
        ("ARC_POOL_SHARE_STARVATION_DISABLE",        "v1 protocol workaround; not needed in V2"),
        ("ARC_MINE_MAX_ITERS",                       "per-sigma iteration caps are disabled in V2"),
    };
    ...
    public static MinerOptions Load(ILogger log)
    {
        ...
        // ARC_POOL_HOST (e.g. a self-hosted gateway). Port defaults to 443
        // when TLS is on (standard), 50052 plaintext otherwise (local dev).
        var host = Env("ARC_POOL_HOST") ?? "pool-v2.akoyapool.com";
        // ARC_POOL_USE_TLS was used by the original packages and some
        // HiveOS flight sheets. Keep it as a compatibility alias while the
        // canonical current name remains ARC_POOL_TLS.
        var tls = ParseBool(Env("ARC_POOL_TLS") ?? Env("ARC_POOL_USE_TLS"), defaultValue: true);
        int port = ParseInt(Env("ARC_POOL_PORT"), defaultValue: tls ? 443 : 50052);

        var wallet = Env("ARC_POOL_WALLET")
                     ?? throw new InvalidOperationException(
                         "wallet address is required — pass --wallet <prl1…> or set ARC_POOL_WALLET");
        var worker = Env("ARC_POOL_WORKER") ?? Environment.MachineName;

        var pool = new PoolOptions(
            Host: host,
            Port: port,
            UseTls: tls,
            TlsInsecure: ParseBool(Env("ARC_POOL_TLS_INSECURE"), defaultValue: false),
            WalletAddress: wallet,
            WorkerName: worker,
            PingIntervalSec:      ParseInt(Env("ARC_POOL_PING_INTERVAL_SEC"), 15),
            HeartbeatIntervalSec: ParseInt(Env("ARC_POOL_HEARTBEAT_INTERVAL_SEC"), 30),
            ...
            // catches a genuinely dead socket. Set ARC_POOL_STREAM_WATCHDOG_SEC
            // to a positive value (e.g. 300) to re-enable the app-layer guard.
            StreamWatchdogSec:    ParseInt(Env("ARC_POOL_STREAM_WATCHDOG_SEC"), 0),
            KeepAlivePingSec:     Math.Max(1, ParseInt(Env("ARC_POOL_KEEPALIVE_PING_SEC"), 10)),
            KeepAliveTimeoutSec:  Math.Max(1, ParseInt(Env("ARC_POOL_KEEPALIVE_TIMEOUT_SEC"), 10)),
            PongTimeoutSec:       ParseInt(Env("ARC_POOL_PONG_TIMEOUT_SEC"), 20),
            OutboundDepthTrip:    ParseInt(Env("ARC_POOL_OUTBOUND_DEPTH_TRIP"), 16));
        ...
        //
        // Similarly the benchmark is mandatory — there is no ARC_BENCH_DISABLE.
        ...
        // explicit ARC_MINE_M/N/K still overrides this everywhere.
        var (defM, defN, defK) = DefaultShareShape(host);
        var mine = new MineOptions(
            M:                 ParseInt(Env("ARC_MINE_M"), defM),
            N:                 ParseInt(Env("ARC_MINE_N"), defN),
            K:                 ParseInt(Env("ARC_MINE_K"), defK),
            // 128 is the consensus floor (rank-penalty softfork, pearl PR #275):
            // the bound scales by 128/rank, so anything above it does more work
            // for the same reward. This default USED to be 256, which was a live
            // trap — ShapeOverridePresent trips on ANY ARC_MINE_* var, so setting
            // only ARC_MINE_M took the override branch and silently carried 256
            // through to the wire at half reward. Nothing rejects such a share,
            // so it never surfaced as an error.
            NoiseRank:         ParseInt(Env("ARC_MINE_NOISE_RANK"), 128),
            MatmulsPerPoll:    10, // probe value; overwritten post-benchmark
            MaxBlocks:         ParseInt(Env("ARC_MINE_MAX_BLOCKS"), 0),
            StatsIntervalSec:  ParseDouble(Env("ARC_MINE_STATS_INTERVAL_SEC"), 30.0),
            WatchdogTimeoutSec:ParseInt(Env("ARC_MINE_WATCHDOG_TIMEOUT_SEC"), 300),
            ...
            TriggerWatchdogSec:ParseInt(Env("ARC_MINE_TRIGGER_WATCHDOG_SEC"), 1800),
            ...
            TriggerWatchdogK:  ParseDouble(Env("ARC_MINE_TRIGGER_WATCHDOG_K"), 20.0),
            FakeTarget:        Env("ARC_FAKE_TARGET") == "1",
            BenchmarkDurationSec:ParseInt(Env("ARC_BENCH_DURATION_SEC"), 10),
            ...
            DisablePong:       ParseBool(Env("ARC_DISABLE_PONG"), defaultValue: false),
            ShapeOverridePresent: MineShapeOverridePresent(),
            CudaGraphIter:     ParseBool(Env("ARC_CUDA_GRAPH_ITER"), defaultValue: false),
            CudaGraphRequired: ParseBool(Env("ARC_CUDA_GRAPH_REQUIRED"), defaultValue: false),
            BM:                ParseInt(Env("ARC_MINE_BM"), 128),
            BN:                ParseInt(Env("ARC_MINE_BN"), 256));

        // ---- GPUs ---------------------------------------------------------
        var gpus = new GpuOptions(
            IndicesRaw: Env("ARC_GPU_INDICES")
                        ?? Env("ARC_GPU_INDEX")   // legacy single-GPU fallback
                        ?? "all");

        // ---- Observability ------------------------------------------------
        var obs = new ObservabilityOptions(
            LogLevel:        Env("ARC_LOG_LEVEL") ?? "Information",
            LogJson:         ParseBool(Env("ARC_LOG_JSON"), defaultValue: false),
            MetricsPort:     int.TryParse(Env("ARC_METRICS_PORT"), NumberStyles.Integer, CultureInfo.InvariantCulture, out var mp) ? mp : null);

        // ---- Session ------------------------------------------------------
        var sess = new SessionOptions(
            FilePath: Env("ARC_SESSION_FILE") ?? DefaultSessionPath());
    ...
    // first, then the legacy ARC_* name silently (existing rigs keep
    ...
    // explicit ARC_MINE_* override is present.
    ...
    /// divergent consensus params. Explicit ARC_MINE_M/N/K/NOISE_RANK env
    ...
    private static bool MineShapeOverridePresent()
        => Env("ARC_MINE_M") is not null
           || Env("ARC_MINE_N") is not null
           || Env("ARC_MINE_K") is not null
           || Env("ARC_MINE_NOISE_RANK") is not null;
