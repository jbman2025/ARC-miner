...
__global__ void __launch_bounds__(
  ...
  static constexpr bool SkipCStore = KTraits::SkipCStore;
  ...
  if (warp_group_idx == 0) {  // Producer
    ...
    if (warp_idx_in_warpgroup == 0) {  // Load A, B in producer warp 0
      ...
      while (work_tile_info.is_valid(scheduler_params)) {
        ...
        // For SkipDenoising the pipeline drain is deferred until after the
        // persistent-CTA loop so successive tiles can keep the producer
        // pipeline warm (producer_tail is a terminal op on the pipeline phase
    ...
    while (work_tile_info.is_valid(scheduler_params)) {
      ...
      // The cvt/denoise/scale/store chain is only needed if we will store C
      // to gmem. When SkipCStore=true (pure miner), C is never read by anyone
      // (the PoW signal is built from tCrC inside mma() and reaches the host
      // via host_signal_header), so all of cvt/denoise/scale/store is dead
      // code. Eliding it (a) frees register pressure for the next tile's mma
      // and (b) lets us drop smem_C from SharedStorage, which frees ~64 KB of
      // smem and unlocks deeper kStages or larger tile_n.
      if constexpr (!SkipCStore) {
