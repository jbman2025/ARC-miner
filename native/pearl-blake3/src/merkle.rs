...
impl MerkleTree {
    ...
    /// Open K **independent**, leaf-major Merkle paths suitable for the
    /// `audit_proof` v1 wire format. Unlike [`get_multileaf_proof`] this
    /// does NOT dedup or merge overlapping paths — every requested index
    /// contributes its own complete chain of `levels = log2(total_leaves)`
    /// sibling digests, in caller-supplied order. The output is the flat
    /// `K × levels × 32` byte buffer the pool consumes verbatim.
    ///
    /// **v1 spec invariant:** `total_leaves` MUST be a power of two; the
    /// non-pow2 promotion semantics are deferred to v2.
    pub fn get_audit_paths(&self, leaf_indices: &[usize]) -> Result<Vec<u8>> {
        let total_leaves = self.num_leaves();
        ensure!(
            total_leaves.is_power_of_two() || total_leaves == 0,
        ...
        if let Some(&bad) = leaf_indices.iter().find(|&&i| i >= total_leaves.max(1)) {
            anyhow::bail!(
                "leaf index {} out of bounds (total_leaves={})",
                bad,
                total_leaves
            );
        }
        ...
        let mut out = vec![0u8; leaf_indices.len() * levels * OUT_LEN];
        if levels == 0 {
            return Ok(out);
        }
        ...
        for (k, &start_idx) in leaf_indices.iter().enumerate() {
            ...
            let block_base = k * levels * OUT_LEN;
            for level in 0..levels {
                let sibling = self.layers[level][idx ^ 1];
                let dst_off = block_base + level * OUT_LEN;
                out[dst_off..dst_off + OUT_LEN].copy_from_slice(&sibling);
                idx >>= 1;
            }
        }
        Ok(out)
...
mod tests {
    ...
    // Print the canonical keyed-BLAKE3 Merkle root of deterministic data, to
    // cross-check the SYCL parallel_tensor_hash kernel (merkle_test.cpp uses
    // the same A[i]=(i*131+7)&0xFF and key[j]=j over 16 KiB).
    #[test]
    fn print_ref_root_for_sycl_crosscheck() {
        let n = 1024 * 16;
        let data: Vec<u8> = (0..n).map(|i| ((i * 131 + 7) & 0xFF) as u8).collect();
        let mut key = [0u8; 32];
        for j in 0..32 { key[j] = j as u8; }
        let root = MerkleTree::new(&data, key).root();
        let hex: String = root.iter().map(|b| format!("{:02x}", b)).collect();
        println!("rust_root={hex}");
    }

    // ---- get_audit_paths ----

    #[test]
    fn audit_paths_empty_indices_returns_empty() {
        let tree = MerkleTree::new(&test_data(16 * CHUNK_LEN), test_key());
        let out = tree.get_audit_paths(&[]).unwrap();
        assert!(out.is_empty());
    }

    #[test]
    fn audit_paths_single_leaf_tree_emits_zero_levels() {
        let tree = MerkleTree::new(&test_data(CHUNK_LEN), test_key());
        let out = tree.get_audit_paths(&[0]).unwrap();
        assert!(out.is_empty(), "single-leaf tree has zero merkle levels");
    }

    #[test]
    fn audit_paths_shape_matches_k_times_levels_times_32() {
        let key = test_key();
        // 64 leaves → 6 levels. K=5 openings → 5*6*32 = 960 bytes.
        let tree = MerkleTree::new(&test_data(64 * CHUNK_LEN), key);
        let out = tree.get_audit_paths(&[0, 1, 2, 31, 63]).unwrap();
        assert_eq!(out.len(), 5 * 6 * 32);
    }

    #[test]
    fn audit_paths_caller_order_preserved_for_duplicates() {
        // The audit_proof wire format requires per-opening blocks in
        // caller order, NOT sorted/deduped (unlike get_multileaf_proof).
        let key = test_key();
        let tree = MerkleTree::new(&test_data(8 * CHUNK_LEN), key);

        let dup = tree.get_audit_paths(&[3, 3]).unwrap();
        let half = dup.len() / 2;
        assert_eq!(&dup[..half], &dup[half..], "duplicate indices ⇒ duplicate blocks");
    }

    #[test]
    fn audit_paths_rejects_non_power_of_two_total_leaves() {
        let key = test_key();
        let tree = MerkleTree::new(&test_data(3 * CHUNK_LEN), key);
        let err = tree.get_audit_paths(&[0]).unwrap_err();
        assert!(err.to_string().contains("power of two"));
    }

    #[test]
    fn audit_paths_rejects_oob_index() {
        let key = test_key();
        let tree = MerkleTree::new(&test_data(4 * CHUNK_LEN), key);
        assert!(tree.get_audit_paths(&[4]).is_err());
    }

    #[test]
    fn audit_paths_recomputes_root_for_each_opening() {
        // Walk each emitted path with parent_cv/root_cv and confirm it
        // ends at tree.root(). This is the canonical correctness check
        // and must hold for every K, every shape.
        let key = test_key();
        let hasher = Blake3Hasher::with_key(key);

        for &total_leaves in &[2usize, 4, 8, 16, 64, 256] {
            let data = test_data(total_leaves * CHUNK_LEN);
            let tree = MerkleTree::new(&data, key);
            let leaf_cvs = tree.leaf_hashes().to_vec();
            let levels = total_leaves.trailing_zeros() as usize;

            let indices: Vec<usize> = (0..total_leaves.min(7)).collect();
            let buf = tree.get_audit_paths(&indices).unwrap();

            for (k, &start_idx) in indices.iter().enumerate() {
                let mut node = leaf_cvs[start_idx];
                let mut idx = start_idx;
                let block_base = k * levels * OUT_LEN;
                for level in 0..levels {
                    let mut sib = [0u8; 32];
                    sib.copy_from_slice(
                        &buf[block_base + level * OUT_LEN..block_base + (level + 1) * OUT_LEN],
                    );
                    let (l, r) = if idx & 1 == 0 { (node, sib) } else { (sib, node) };
                    node = if level == levels - 1 {
                        hasher.root_cv(&l, &r)
                    } else {
                        hasher.parent_cv(&l, &r)
                    };
                    idx >>= 1;
                }
                assert_eq!(node, tree.root(), "path #{k} (leaf {start_idx}) failed to reach root for total_leaves={total_leaves}");
            }
        }
    }
