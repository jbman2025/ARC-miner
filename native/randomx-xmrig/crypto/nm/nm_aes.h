...
/* ARC PATCH 2/2 (MSVC): upstream gates AES-NI on __x86_64__ && __AES__, which
 * only GCC/Clang define (the fork builds Windows with MinGW-GCC). MSVC defines
 * _M_X64 and has no __AES__ — it exposes the AES intrinsics unconditionally on
 * x64 — so without this the build silently falls back to the software AES path:
 * bit-identical output, but far slower. */
#if defined(__x86_64__) && defined(__AES__)
  #define NM_AESNI 1
  #include <wmmintrin.h>
  #include <emmintrin.h>
#elif defined(_M_X64) && !defined(__GNUC__)
  #define NM_AESNI 1
  #include <wmmintrin.h>
  #include <emmintrin.h>
#else
